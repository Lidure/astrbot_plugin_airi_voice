...
class AiriVoice(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        self._load_user_added_voices()  # 新增：加载用户添加的语音
    ...
    async def voice_add(self, event: AstrMessageEvent, name: str):
        ...
        save_path = self.user_added_dir / save_name  # 改成 user_added_dir
    
        try:
            with open(save_path, 'wb') as f:
                f.write(voice_data)
            logger.info(f"[AiriVoice] 成功保存用户语音：{save_name} → {save_path}")
    ...
    async def voice_delete(self, event: AstrMessageEvent, name: str):
        ...
        if not str(file_path.resolve()).startswith(str(self.user_added_dir.resolve())):
            yield event.plain_result("⚠️ 只能删除通过 /voice.add 添加的语音，本地 voices/ 和网页上传的文件请手动管理")
            return
